...
from usfgan.bin.sigproc import spec_to_ar, lp_synthesis_filter

from usfgan.bin.melspec import inv_melspectrogram
...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        print("_train_step_ function")
        # parse batch
        x, y = batch

        #print("generator len x", len(x))
        ...
        #y_fake, s = self.model["generator"](z, c, df, logmsp)[:3]
        print("generator forward.........finish")
        #print("y_fake", len(y_fake))
        #print("y_fake", type(y_fake))
        #torch.set_printoptions(profile="full")
        #print("y_fake", y_fake)
        
        ##### For Linear Prediction Filter MODIFICATION ##################
        ##Convert mel spec to its inverse

        #print("logmsp",logmsp)
        #print("logmsp size", logmsp.size())
        
        #M = logmsp.numpy()
        #logmsp_inv = inv_melspectrogram(MS=logmsp, num_mels=80, sample_rate=16000, num_freq=513, ref_level_db=20, clip=False, fmax=None)
        #print("M type", type(M))
        #print("M type", type(M))
        #S = librosa.feature.inverse.mel_to_stft(M, sr=16000, n_fft=1024, power=2.0)
        M = torch.reshape(logmsp, (-1,))
        LOGMSP_INV = torch.stft(M, 1024, 80, 1024,  window=torch.hann_window(1024, periodic=True) , center=True, onesided=True, return_complex=True)
        

        #logmsp_inv = torch.from_numpy(logmsp_inv)
        #print("logmsp_inv", LOGMSP_INV)
        #print("logmsp_inv", LOGMSP_INV.size())

        #######Get LP coefficients##########
        
        lpc = spec_to_ar(LOGMSP_INV)
        #print("type lpc", lpc.size())

        ###Print y_fake before synthesis filter############

        print("excitation signal  size: ", s.size())
        print("y fake  size: ", y_fake.size())
       #print("y fake: ", y_fake)

        #####################LP Synthesis Filter#################################

        y_fake = lp_synthesis_filter(y_fake, lpc)
        #y_test = lp_synthesis_filter(y_fake, lpc)

        ##########################################################################
        print("After passing to LP filter.......")

        print("y_fake  size: ", y_fake.size())
        #print("s: ", y_fake)
        print("y real size: ", y_real.size())
        #print("y real: ", y_real)
    ...
    def _generate_and_save_intermediate_result(self, batch):
        ...
        print("Generate and save intermediate result")
        ...
        #z_batch, c_batch, df_batch, logmsp_batch = x_batch[:4]

        # generator forward
        h_batch, n_batch, a_batch = None, None, None
        #h_batch, n_batch, a_batch = None, None, None, None
        if isinstance(
            ...
            y_fake_batch, s_batch, h_batch, n_batch, a_batch = self.model["generator"](
                z_batch, c_batch, df_batch, , logmsp
            )
            """
            y_fake_batch, s_batch, h_batch, n_batch, a_batch = self.model["generator"](
...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        y_batch, c_batch, df_batch, f0_batch, contf0_batch, logmsp_batch = [], [], [], [], [], []
        #print("len batch", len(batch))
        for idx in range(len(batch)):
  
            x, c, df, f0, contf0, logmsp = batch[idx]
            #x, c, df, f0, contf0 = batch[idx]
            self._check_length(x, c, df, f0, contf0, logmsp, 0)
            ...
            if len(c) - 2 * self.aux_context_window > self.batch_max_frames:
                ...
                #newmodif
                logmsp = logmsp[start_frame : start_frame + self.batch_max_frames]
                self._check_length(
                    y,
                    c,
                    df,
            ...
            #newmodif
            logmsp_batch += [logmsp.astype(np.float32).reshape(-1, 1)]  # [(T', 1), ...]
        ...
        #newmodif
        logmsp_batch = torch.FloatTensor(np.array(logmsp_batch)).transpose(2, 1)
        #print("logmsp_batch", logmsp_batch)
        ...
        return (in_batch, c_batch, df_batch, f0_batch, logmsp_batch), y_batch

    def _check_length(self, x, c, df, f0, contf0, logmsp, context_window):
