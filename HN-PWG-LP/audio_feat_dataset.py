...
class AudioFeatDataset(Dataset):
    ...
    def __init__(
        ...
        #logger.info(f"Feature type : {self.logmsp}")
    ...
    def __getitem__(self, idx):
        ...
        #log mel spectogram
        logmsp = read_hdf5(to_absolute_path(self.feat_files[idx]), "/logmsp")
        #print("logmsp (aduio_feat_dataset)")
        #audio_mel, logmsp = validate_length(audio, logmsp, self.hop_size)

        #self.logmsp = logmsp

        if self.return_filename:
            #items = self.feat_files[idx], audio, aux_feats, df, f0, contf0
            items = self.feat_files[idx], aux_feats, df, f0, contf0, logmsp
        else:
            #items = audio, aux_feats, df, f0, contf0
            items = audio, aux_feats, df, f0, contf0, logmsp
...
class FeatDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_filename:
            ...
            items = aux_feats, df, f0, contf0, logmsp
